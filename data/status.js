...
async function adoptPet(user_id, pet_id) {
  try {
    const sql = `INSERT INTO user_has_pet (type, saved, user_id, pet_id) VALUES ('Adopted', CURRENT_TIMESTAMP, '${user_id}', '${pet_id}')`;
...
async function fosterPet(user_id, pet_id) {
  try {
    const sql = `INSERT INTO user_has_pet (type, date, user_id, pet_id) VALUES ('Fostered', CURRENT_TIMESTAMP, '${user_id}', '${pet_id}')`;
